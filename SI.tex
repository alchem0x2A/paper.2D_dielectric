...
\documentclass[journal=ancac3,email=true,hyperref=true,keywords=false]{achemso}
%manuscript=suppinfo 
\usepackage{times}
\usepackage{float}
\usepackage{amsmath}
...
%\usepackage{xr}
\usepackage{xr-hyper} 
\externaldocument[main-]{paper_v120719}
...
% Here we will show the results of dielectric properties calculated
% using many-body Green function method (G$_0$W$_{0}$) and with
% electron-hole interactions at the level of the Bethe-Salpeter equation
% (G$_0$W$_{0}$--BSE). Frequency-dependent dielectric functions
% $\varepsilon_{\mathrm{SL}}(\omega)$ at both G0W0 and G0W0--BSE levels
% are calculated using VASP, with a cutoff energy of XXX and k point
% density of YYY. Local field effect is included in both cases: local
% field correction is applied to independent-particle energies in G0W0
% at RPA level with VASP TAG \texttt{LRPA=.TRUE.}, while the full
% polarizability is calculated using BSE in G0W0-BSE
% \todo[inline]{@Declan, could you help modify this part?}
...
\begin{equation}
    G^{(0)}(\mathbf{r},\mathbf{r}',\omega)=\sum_n\frac{\phi_{\mathrm{n}\mathbf{k}}^{(0)}(\mathbf{r})\phi_{\mathrm{n}\mathbf{k}}^{*(0)}(\mathbf{r'})}{\omega-\epsilon_\mathrm{n}-+i\eta\mathrm{sgn}(\epsilon_\mathrm{n}-\epsilon_\mathrm{F})}
\end{equation}

Here $\epsilon_n(\mathbf{k})$ are the DFT eigenenergies at \textbf{k}, $\epsilon_F$ the Fermi energy, $\omega$ the frequency, $\ket{u_{n\mathbf{k}}}$ the cell periodic Bloch functions, $\phi_{n\mathbf{k}}$ are the one-electron orbitals and $\eta$ is an infintesimal complex shift. It can be seen from equation \ref{eq:dft-dielectric} there is a clear volume dependence on $\Omega$ of the dielectric function from the preceding DFT calculation. Thus, when carrying out a calculation on a slab, the dielectric function will vary with the vacuum spacing used.

Using the dielectric function we can calculate the screened Coulomb interaction:
\begin{equation}
    W=\epsilon^{-1}\nu
\end{equation}
where $\nu$ is the bare Coulomb interaction given by
$e^2$/$|\mathbf{r}-\mathbf{r}'|$. Due to the
1/$|\mathbf{r}-\mathbf{r}'|$ term, images in the non-periodic
direction have a long-range spurious interaction which varies with
vacuum spacing. From the screened Coulomb interaction and
non-interacting Green's function it is possible to calculate the
...
E^{\mathrm{QP}}_{\mathrm{n}\mathbf{k}}=\Re\Big[\braket{\phi_{\mathrm{n}\mathbf{k}}|-\frac{1}{2}\Delta+V_{\mathrm{ext}}+V_\mathrm{H}+\Sigma(\epsilon_{\mathrm{n}\mathbf{k}}^{\mathrm{DFT}})|\phi_{\mathrm{n}\mathbf{k}}}\Big]
\end{equation}
Therefore a new set of eigenergies, E$_{n\mathbf{k}}^{QP}$ are found. Using E$_{n\mathbf{k}}^{QP}$ it is then possible to recalculate the dielectric function using the quasi-particle eigenergies as well as the DFT eigenfunctions.
...
%
%
%For bulk systems, the most accurate simulations are based on BSE approach for 
%the optical properties. As excitonic effects including electron-hole interactions are taken 
%into account 
% 
%This is because the calculation of the frequency-dependent
%optical properties of a material involves a summation over the valence
%and conduction states. 
%
%Thus, the accuracy of these states determines
%the calculated optical properties. 
%
%Within Kohn-Sham density functional
...
%on the homogenous electron gas. 
%% 
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\linewidth]{img/SI-alpha-convergence.pdf}
...
\begin{figure}[htbp]
  ...
  \caption{Validation of the linear relation between \textbf{a,}
    ...
    Ref.~\citenum{Haastrup_2018} corresponding to main text Figure \ref{main-fig-3}{\bf c}
...
semiconducting 2D materials from Ref.~\citenum{Haastrup_2018} 
with a GW bandgap
...
Ref.~\citenum{Haastrup_2018} as a function of minimal and direct bandgap calculated
...
\begin{figure}[htbp]
  ...
  \caption{$\alpha_{\mathrm{2D}}^{\parallel}$ as function of minimal and direct
    ...
    \textbf{a,} PBE, \textbf{b,} HSE and \textbf{c,} GW of Ref.~\citenum{Haastrup_2018}.}
...
from Ref.~\citenum{Haastrup_2018} with
...
\begin{figure}[htbp]
  ...
  \caption{$(4\pi\varepsilon_{0})/\alpha_{\mathrm{2D}}^{\parallel}$ as a function of
    $E_{\mathrm{g}}^{\mathrm{opt}}$ from Ref.~\citenum{Haastrup_2018}.}
  \label{fig:opt}
\end{figure}
...
The relatively large size of Ref.~\citenum{Haastrup_2018} database allows us
...
\begin{figure}[htbp]
  ...
  \caption{Relation between 2D polarizabilities and the effective
    carrier mass from Ref.~\citenum{Haastrup_2018}. 
...
$\sigma_{\mathrm{2D}}^{\mathrm{v}}/E_{\mathrm{g}}^{2}$ using Ref.~\citenum{Haastrup_2018} database, as shown in Figure \ref{fig:plasma}.
\begin{figure}[htbp]
  \centering
  ...
  \caption{Calculated $\alpha_{\mathrm{2D}}^{\parallel}$ as a function of
    $\sigma_{\mathrm{2D}}^{\mathrm{v}}/E_{\mathrm{g}}^{2}$ using data from Ref.~\citenum{Haastrup_2018}. The broken line shows the theoretical prediction
