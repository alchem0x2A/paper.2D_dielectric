...
$\alpha_{\mathrm{2D}}^{\parallel}$ and $\alpha_{\mathrm{2D}}^{\perp}$ at 0 K can be unified by the
same equation:
\begin{equation}
  \label{eq:alpha-RPA}
...
For the in-plane component, $\alpha_{\mathrm{2D}}^{\parallel}$, $e^{-i\mathbf{qr}}$
...
\begin{equation}
  ...
  \alpha_{\mathrm{2D}}^{\perp} \propto \delta^{2}
\end{equation}
Apart from the power of $\delta$, this explains our observation
$\alpha_{\mathrm{2D}}^{\perp}$ is related with the thickness of a 2D material. We
...
such approach will always yield $\alpha_{\mathrm{2D}}^{\perp} \propto \delta^{2}$
regardless of the Hamiltonian used. To accurately model the
$\alpha_{\mathrm{2D}}^{\perp}$ we need a better model for the confinement in
...
$\alpha_{\mathrm{2D}}^{\perp}$, supporting our findings in the main text.

The dependency of $\alpha_{\mathrm{2D}}^{\perp}$ on the thickness of a 2D material,
...
of $\alpha_{\mathrm{2D}}^{\perp}$, and reproduces the right coefficient between
$\delta_{\mathrm{cov}}$ and $\alpha_{\mathrm{2D}}^{\perp}$. Finally, the schematic
