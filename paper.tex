...
metals, semiconductors and insulators.  \todo[inline]{I shortened the
  sentences. However, can we really "classify" the material only by
  $\varepsilon$? For instance HfO2 has large $\varepsilon$ but also
  large bandgap}
...
($|\boldsymbol{E}|=0.01 \mathrm{V\cdot \AA}^{-1}$) extends to
...
of theory are needed to predict bandgaps.  We also show that the
proposed Eq. \ref{eq:2D-Moss-para} behaves equivalently well when
using the experimentally accessible optical bandgap of 2D materials (
Supplementary Figure \ref{SI-fig:opt}).
...
material (see Supplementary Section
\ref{SI-ssec:theory-1-perp-fundamental}). Indeed, the universal
...
anisotropy indices $\eta$ of common semiconducting materials in other
