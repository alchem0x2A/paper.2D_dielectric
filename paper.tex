...
Section SXX) \todo{note}, we show that
$\alpha_{\mathrm{2D}}^{\parallel}$ and $\alpha_{\mathrm{2D}}^{\perp}$
...
$E_{\mathrm{g}}$ is observed (see Supplementary Section S2.2), which
appears to be in good agreement with our polarizability-based
theoretical model (Supplementary Section SXX \todo{note}). We then
examine Eqs. \ref{eq:2D-Moss-para} and \ref{eq:2D-Moss-perp} using the
...
corresponds well with theoretical investigations
\citenum{Jiang_2017_Eg_Eb} (details see Supplementary Section SXX
\todo{format Jiang's paper into SI}).
% From Ref. \citenum{Jiang_2017_Eg_Eb}, the slope between
...
% studied.
% We observe that materials with lower $E_{\mathrm{g}}$ ($<$4
...
% 2D materials \cite{Olsen_2016_hydrogen,Jiang_2017_Eg_Eb}.
We also discover that the linearity between
$(4 \pi \varepsilon_{0})/\alpha_{\mathrm{2D}}^{\parallel}$ and $E_{\mathrm{g}}$
(measured by the $R^{2}$ value) is higher when the bandgap is
calculated using the HSE06 hybrid functionals, compared with that from
the Perdew--Burk--Ernzerhof (PBE) exchange correlations (details see
Supplementary Section S2 and Supplementary Figure S5), which can be
...
$\alpha_{\mathrm{2D}}^{\parallel}$ and $E_{\mathrm{g}}$.
\todo[inline]{check the values for optical bandgap}
...
scaling relation for $\alpha^{\perp}$ in Figure \ref{fig-3}c indicates
that the quantity $\alpha^{\perp}/\varepsilon_{0}$ measurable via
...
Supplementary Section SXX \todo{note}). In other words, the proposed
...
% \end{aligned}
% \end{equation}
% which recovers the 3D Moss relation.
...
predicted by Eq. \ref{eq:3D-para} align along the line $y=x$ , with a
linear regression slope of 1.01 and $R^2$ of 0.97. On the other hand,
...
$E_{\mathrm{g}}$ reduces. The difference for the prediction of
...
$\alpha_{\mathrm{bulk}}^{\perp}$ as function of $\alpha^{\perp}$ and
