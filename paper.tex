...
placed in the xy-plane of a periodically repeating superlattice (SL)
with a length $L$ along the z-direction. The macroscopic dielectric
...
the 2D material is placed in vacuum, such that $\kappa^{pp} = 1$
and $\kappa^{pq} = 0$, we can distinguish two components of the
...
extends to ~12 \AA{} along the z-direction. When $L$ is
...
\ref{fig-1}c \todo{change order!}, respectively. We also observe
...
\rho(\boldsymbol{r}) \boldsymbol{r}^{p}d^{3} \boldsymbol{r}/A$,
...
\begin{equation}
  ...
  = \lim_{\mathbf{q} \to 0} 1 + \frac{2e^{2}}{\varepsilon_{0} |\boldsymbol{q}|^{2} \Omega}
  \sum_{\mathrm{k, c, v}}
...
0.84. From Ref. \citenum{Jiang_2017_Eg_Eb}, the slope between
