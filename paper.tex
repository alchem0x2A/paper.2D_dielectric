...
with elemental interactions, e.g. electron-electron. 
...
external electric field. For the majority of the 2D nanosheets, the
...
components of $\kappa$, namely the in-plane
...
$\sim{}$5--6 \AA{} into the vacuum (Figure \ref{fig-1}{\bf a} and
...
techniques, G$_{\rm 0}$W$_{0}$, invariably give alike
results (see Supplementary Figure \ref{SI-fig:GW-PBE-alpha}). 
We have also undertaken similar analysis into the frequency regime 
for $\varepsilon^{\parallel}_{\mathrm{SL}}(\omega)$ and
$\varepsilon^{\perp}_{\mathrm{SL}}(\omega)$ where despite of the 
method utilized from PBE, G$_{\rm 0}$W$_{0}$ up 
to taking into account electron-hole excitations at the level of the 
Bethe-Salpeter equation (G$_{\rm 0}$W$_{0}$--BSE) the dependence on $L$
is still present (see Supplementary Section \ref{SI-ssec:gw} for a systematic study). 
These results indicate that quantities that depend 
on $\varepsilon^{\parallel}_{\mathrm{SL}}(\omega)$ and
$\varepsilon^{\perp}_{\mathrm{SL}}(\omega)$, for instance, 
the optical absorption, suffer the
same deficiencies.  
%
...
comparison with other methods, simulations at the frequency-dependent domain
 can be found in Supplementary Section \ref{SI-ssec:gw}. 
...
Section \ref{SI-sec:theory-1} for details), 
we can show that
$\alpha_{\mathrm{2D}}^{\parallel}$ and $\alpha_{\mathrm{2D}}^{\perp}$
can be written as: 
%
...
the intrinsic thickness of the layer (Fig. \ref{fig-1}{\bf a}), and
$C^{\parallel} = \frac{Ne^2}{2 \pi}$ \cite{Jiang_2017_Eg_Eb}, where
...
Despite of the simplicity of
Eqs. \ref{eq:2D-Moss-para} and \ref{eq:2D-Moss-perp}, they generalize
a direct relationship between the polarizability and the electronic
and structural properties for any 2D material in a new framework.  We
can show that these equations are valid for the current library of
known layered materials involving different lattice symmetry, element
composition, optical and electronic properties.
%
...
\ref{SI-sec:pol-2D-Eg}). 
%It is worth mentioning that materials with
...
%
% 
We then examine Eqs. \ref{eq:2D-Moss-para} and \ref{eq:2D-Moss-perp}
using the polarizabilities by first-principle calculations.  Figure
\ref{fig-3}{\bf c} shows
...
Perdew--Burk--Ernzerhof (PBE) %exchange correlation functional
...
of theory are needed to predict bandgaps. See Supplementary Section \ref{SI-sec:gpaw}
for a benchmark of Eqs.~\ref{eq:2D-Moss-para} and \ref{eq:2D-Moss-perp}
using different bandgaps, databases, and levels of calculation. 
...
This result indicates that $\alpha_{\mathrm{2D}}^{\perp}$ has a strong
physical meaning through the geometry of the layer.  Similar to the
molecular polarizability which characterizes the volume of the electron
...
\ref{SI-ssec:theory-1-perp-fundamental} shows an electrostatic model 
of this behavior. Indeed, the universal
scaling relation for $\alpha_{\mathrm{2D}}^{\perp}$ in Figure
\ref{fig-3}{\bf d} indicates that the quantity
...
(\textit{x}-axis) as shown in Figure \ref{fig-4}{\bf b} and \ref{fig-4}{\bf c}. 
...
augmented wave (PAW) approach with GW pseudopotentials
...
guarantees that absolute energies were converged to a few meV and
the stress tensor to within 0.01 kBar.  Calculation of the
...
while the dielectric properties of bulk counterparts of Ref.~\citenum{Haastrup_2018}
are calculated at PBE level with a k-point density of 10~\AA$^{-1}$. 
...
\begin{figure}[H]
\centering
\includegraphics[width=0.79\linewidth]{img/fig1.pdf}
\caption{\label{fig-1} \textbf{2D polarizability as the true
  ...
  \ref{eq:alpha-perp-def}, respectively. HSE06 functional is used in all plots.}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.74\linewidth]{img/fig2.pdf}
